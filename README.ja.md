...
DaemonSet として動作し、`/var/log/pods` を監視して各 Pod ログを同一ファイルシステム上の保全ディレクトリに**ハードリンク**する。これにより kubelet が元ファイルを削除してもバイト列は生き続ける。クリーンアップループはログエージェント（fluent-bit）の tail DB を**読み取り専用**で参照し、エージェントが読み終えた保全ファイルだけを削除する。未確認のファイルは age しきい値によるフォールバックで削除される。
...
| `config.cleanupMaxAgeMin` | `5` | `.gz` 以外の孤児の age しきい値 |
...
`METRICS_PORT`（デフォルト `9113`）の `/metrics` で Prometheus エンドポイントを提供する。chart はデフォルトでアノテーションベースの scraping を有効化する。[仕様 §4.2](docs/ja/specification/04-operations.md#42-可観測性)を参照。
