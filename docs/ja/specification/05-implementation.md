...
**サポートする fluent-bit バージョン（tail DB スキーマ・マトリックス）。** この読み取りは fluent-bit の `in_tail_files` テーブルと、その `inode`・`offset`・`name` 列のみに依存する。ここで `offset` は fluent-bit が読み取ったバイト数（ファイルサイズと比較可能）である。クエリは `SELECT *` ではなくこの 3 列を**名前で指定**するため、追加的なスキーマ変更の影響を受けない。

| fluent-bit | `in_tail_files` の列 | tail DB 読み取り |
| --- | --- | --- |
| **1.x – 4.x**（リリース済みの全メジャー） | `id, name, offset, inode, created, rotated` | **サポート** — これらのメジャー間でスキーマは完全に同一（`plugins/in_tail/tail_sql.h` の `v1.9.10`・`v2.2.3`・`v3.0.7`・`v3.1.9`・`v3.2.0`・`v4.0.0`・`v4.2.3` で確認） |
| **5.x**（`v5.0.0` 以降） | 上記 **+** `offset_marker`・`offset_marker_size` | **サポート** — 追加された 2 列は名前指定クエリでは無視される（`tail_sql.h` の `v5.0.9` で確認） |
| `inode` 列を欠く DB、または fluent-bit 以外のスキーマ | 認識不可 | クエリがエラーになる。カウントされ（`pod_log_preserver_fluentbit_db_errors_total`）、DB はスキップされ、その孤児は age ベースのクリーンアップにフォールバックする |

e2e ハーネスは実行検証済みバージョンとして `fluent/fluent-bit:3.1.9` を固定する。表のバージョン非依存性は、各メジャーのスキーマを構築して読み取りを検証するユニットテストのサポートマトリックスで担保する。フォールバックの方向は常に安全であり、削除は遅延するだけで早すぎることはない。したがって fluent-bit 以外や将来非互換なスキーマは、誤った読み取りではなく age のみのクリーンアップに縮退する（プラグイン可能なリーダーという未解決事項は §7.3 を参照）。
