...
DaemonSet として動作する単一バイナリの Go プログラム。実装は文字どおりの単一ファイルではなく、1 つの `package main` を関心ごと（設定・ログ・メトリクス・inotify 監視・保全・tail DB 読み取り・クリーンアップ・検証）のファイルに分割している。3 つの並行ループがメモリ上のメトリクスカウンタを共有し、context 経由でシャットダウンを協調させる。
