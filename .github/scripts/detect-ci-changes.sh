...
# `<their-flag> || infra`, a deliberately conservative over-trigger. Unlike
# node-rotation-controller there is no api/, config/, aqua-policy.yaml, or local
# aqua registry here, so those patterns are absent.
set -euo pipefail
